...
function autocompletesupplier(idd) {
    ...
    inp.addEventListener("input", function(e) {
        var a, b, i, val = this.value;
        $.ajax({
            url: '/api/resource/Supplier/?fields=["name", "supplier_name", "default_currency"]&limit_page_length=0',
            type: 'get',
            dataType: "json",
            success: function(r) {
                for (i = 0; i < r.data.length; i++) {
                    ...
                    if (r.data[i]['supplier_name'].toUpperCase().includes(val.toUpperCase())) {
                        ...
                        /*insert a input field that will hold the current array item's value:*/
                        b.innerHTML += "<input type='hidden' class='itmname' value='" + r.data[i]['name'] + "'><input type='hidden' class='itmid' value='" + r.data[i]['name'] + "'><input type='hidden' class='itmcurrency' value='" + r.data[i]['default_currency'] + "'><span style='display:none'   class='idesc'>" + r.data[i]['supplier_name'] + "</span>";
                        /*execute a function when someone clicks on the item value (DIV element):*/
                        b.addEventListener("click", function(e) {
                            /*insert the value for the autocomplete text field:*/
