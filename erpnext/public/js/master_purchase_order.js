			...
			success: function( data ) {
				...
				if(data.data.master_purchase_order_item.length > 0) 
				{
					...
					for (var i = 0; i < data.data.master_purchase_order_item.length; i++) 
					{
						...
						var splcd = {'name': dta.name, 'qty': dta.qty, 'received_qty': dta.received_qty, 'price': dta.price, 'supplier_name': dta.supplier_name, 'supplier': dta.supplier , 'col_number' : dta.col_number};
...
//console.log(TableArray);
				...
				var t2 = '<tr data-row="2"><td data-row="2" data-col="0"></td><td data-row="2" data-col="1">Items</td><td data-row="2" data-col="2">Description</td><td data-row="2" data-col="3"><div class="text-span">Pallets </div><div class="text-span"> Gross W</div></td><td data-row="2" data-col="4">Net W</td>';
				var titems = '';
			...
			for(var k=1; k < TableArray[v].length; k++)
			{
				...
				t1 = t1 + '<td data-row="1" data-col="'+ dataCol1 +'"><div class="autocomplete"><input type="hidden" class="isupplierid" name="isupplierid" value="'+ TableArray[1][k].supplier +'"><input type="text" id="isupplier'+ dataCol1 +'" class="isupplier"  name="isupplier'+ dataCol1 +'" value="'+ TableArray[1][k].supplier +'"></div></td>';
				...
				$("#myTable").find(".sitems").each( function()
				{
					...
					if(row.find('.item_code').val() != '') 
					{
						...
						for(var i = 5; i < (colCount - 5); i++) 
						{
							...
							var listitm = {'name': iname, 'qty': iqty, 'price': iprice, 'weight': row.find('.weight').val(), 'weight_per_unit': row.find('.weight').val(), 'gross_weight': row.find('.gross_weight').val(), 'uom': row.find('.uom').val(), 'description': row.find('.idesc').val(), 'purchase_pallets': row.find('.purchase_pallets').val(), 'item_code': row.find('.item_code').val(), 'item_name': row.find('.sitems').val(),'received_qty' : rqty, 'supplier_name': $('#isupplier'+i).val(), 'supplier': $('#isupplier'+i).val(), 'row_number': vindex , 'col_number': col_number};
